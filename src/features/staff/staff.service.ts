...
// Reuse your existing results helpers if you have them:
// import { resultsService } from "../results/results.service";
// import { gradeFromTotal, computeScoreTotal } from "../results/results.grades";
...
function assertStaff(actor: StaffActor) {
  const ok =
    actor.orgRole === OrgRole.TEACHER ||
    actor.orgRole === OrgRole.EXAM_OFFICER ||
    actor.orgRole === OrgRole.SCHOOL_ADMIN;
  if (!ok) throw new ForbiddenError("Staff access only");
}

function isExamOrAdmin(actor: StaffActor) {
  return (
    actor.orgRole === OrgRole.EXAM_OFFICER ||
    actor.orgRole === OrgRole.SCHOOL_ADMIN
  );
...
export class StaffService {
  ...
  async getMyDashboard(actor: StaffActor) {
    assertStaff(actor);
  ...
  async listMyAssignments(actor: StaffActor) {
    assertStaff(actor);
  ...
  async assertCanEnterScores(
    ...
    if (isExamOrAdmin(actor)) {
      ...
      if (!cs) {
        throw new ForbiddenError("Subject is not on this class curriculum");
      }
      return;
    }
  ...
  async assertCanViewClass(actor: StaffActor, classId: string) {
    if (isExamOrAdmin(actor)) return;
    ...
    if (cls.classTeacherId === actor.userId) return;
    ...
    if (!teaches) {
      throw new ForbiddenError("You do not teach in this class");
    }
  }

  // ─── Form teacher / class view ───────────────────────────────────────────

  async getClassDetail(actor: StaffActor, classId: string) {
    assertStaff(actor);
    ...
    const mySubjects = await prisma.classSubjectTeacher.findMany({
      where: {
        organizationId: actor.organizationId,
        userId: actor.userId,
        classId,
  ...
  async getClassRoster(actor: StaffActor, classId: string) {
    assertStaff(actor);
    await this.assertCanViewClass(actor, classId);
  ...
  async getEntrySheet(
    actor: StaffActor,
    query: { termId: string; classId: string; subjectId: string }
  ) {
    assertStaff(actor);
  ...
  async bulkUpsertScores(actor: StaffActor, body: BulkScoresBody) {
    assertStaff(actor);
    ...
    if (!isExamOrAdmin(actor)) {
      ...
      if (published > 0) {
        throw new ForbiddenError(
          "Some results are published. Ask exam officer or admin to edit."
        );
      }
    }
    ...
    for (const row of scores) {
      if (!validIds.has(row.studentId)) {
        throw new BadRequestError(
          `Student ${row.studentId} is not in this class`
        );
      }
    }
  ...
  async getAssignmentProgress(
    ...
    const withScore = await prisma.score.count({
      where: {
