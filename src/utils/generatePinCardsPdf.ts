...
/**
 * NOTE: `PinCard` should already exist in your codebase — import it from
 * wherever it's defined instead of this local copy. It's included here only
 * so this file type-checks standalone. Delete this block once you wire in
 * your real import, and adjust field names below (`serial` / `pin`) to match.
 */
export type PinCard = {
  id: string;
  serial: string;
...
export type SchoolBrandForPinPdf = {
  schoolName: string;
  /** e.g. "#0A1628" or "0A1628" */
  primaryColor?: string | null;
  /** e.g. "#C9A227" */
  accentColor?: string | null;
};

export type GeneratePinCardsPdfOptions = {
  cards: PinCard[];
  termLabel: string;
  brand?: SchoolBrandForPinPdf;
  schoolName?: string;
  password: string;
  protectUrl?: string;
  accessToken?: string;
...
type ResolvedBrand = {
  schoolName: string;
  primary: string; // normalized "#RRGGBB"
...
export async function generatePinCardsPdf(
  options: GeneratePinCardsPdfOptions
): Promise<Buffer> {
  const {
    cards,
    termLabel,
  ...
  if (!cards || cards.length === 0) {
    throw new Error(
      "generatePinCardsPdf: `cards` must contain at least one card."
    );
  }
 ...
 */
async function generateQrCodesForCards(
  cards: PinCard[],
  protectUrl: string,
  accessToken?: string
...
function getSharedStyles(brand: ResolvedBrand): string {
  return `
...
function buildFrontPageHtml(brand: ResolvedBrand, termLabel: string): string {
  return `
  <section class="sheet front-page">
    <div class="card">
      <div class="card__main">
        <div class="card__watermark">${escapeHtml(
          firstLetter(brand.schoolName)
        )}</div>
  ...
  return pages
    .map((pageCards, pageIndex) => {
      const cardsHtml = pageCards
        .map((card) =>
          buildBackCardHtml(card, brand, termLabel, qrByCardId.get(card.id))
        )
        .join("");
      ...
      <section class="sheet back-page">
        <div class="back-page__header">
          <span><strong>${escapeHtml(
            brand.schoolName
          )}</strong> &middot; ${escapeHtml(termLabel)}</span>
      ...
      <div class="card__main">
        <div>
          <div class="back-card__top">
            <div class="back-card__monogram">${escapeHtml(
              firstLetter(brand.schoolName)
            )}</div>
            <div>
              <div class="back-card__school">${escapeHtml(
                brand.schoolName
              )}</div>
...
async function encryptPdfBuffer(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pin-cards-"));
...
export function generateCardSerial(prefix = ""): string {
  const random = crypto.randomBytes(4).toString("hex").toUpperCase();
  return `${prefix}${random}`;
}

/** Generates a fixed-length numeric PIN (override as needed). */
export function generateCardPin(length = 6): string {
  const max = 10 ** length;
